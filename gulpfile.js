...
gulp.task('server', ['build'], function(){
    browser.init({server: './_web', port: port});
});
...
gulp.task('setupBulma', function() {
    ...
    gulp.src([nodepath + 'semaintic-ui-sass/*.scss']).pipe(gulp.dest('semantic-ui-sass/'));
    gulp.src([nodepath + 'semaintic-ui-sass/**/*.scss']).pipe(gulp.dest('semantic-ui-sass/'));
});
...
gulp.task('copy-plugins', function() {
    //Modal video
    gulp.src([nodepath + 'modal-video/js/**/*']).pipe(gulp.dest('_web/assets/js/modalvideo/'));
    gulp.src([nodepath + 'modal-video/css/**/*']).pipe(gulp.dest('_web/assets/js/modalvideo/'));
    //Particles js
    gulp.src([nodepath + 'particlesjs/dist/**/*']).pipe(gulp.dest('_web/assets/js/particlesjs/'));
    //Aos
...
gulp.task('copy', function() {
    ...
    gulp.src(['css/*.css']).pipe(gulp.dest('_web/assets/css/'));
});
...
var sassOptions = {
    ...
    includePaths: [nodepath + 'semantic-ui-sass/scss']
};
...
gulp.task('compile-sass', function () {
    ...
    //Watch me get Sassy
    return gulp.src('./semantic-ui-scss/semantic-ui.scss')
        .pipe(sourcemaps.init())
        .pipe(sass(sassOptions).on('error', sass.logError))
        .pipe(postcss(processors))
        .pipe(sourcemaps.write())
        .pipe(gulp.dest('./_web/assets/css/'));
});
...
gulp.task('compile-scss', function () {
    ...
    return gulp.src([
      ...
      './scss/style.scss',
      './semantic-ui-sass/semantic-ui.scss',
  ])
        .pipe(sourcemaps.init())
        .pipe(sass(sassOptions).on('error', sass.logError))
        .pipe(postcss(processors))
...
gulp.task('copy-images', function() {
    gulp.src('images/**/*')
        .pipe(gulp.dest('./_web/assets/images/'));
});
